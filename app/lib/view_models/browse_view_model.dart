...
class BrowseViewModel extends ChangeNotifier {
  ...
  Future<void> cacheRecommendationsSnapshot(
  ...
  Future<void> persistCatalogAndRecommendations(
  ...
  Future<void> _cacheForYouRecommendations(
    List<Listing> recommendations,
  ) async {
    final box = await Hive.openBox<List>('fyp_cache');
    await box.put(
      'cached_fyp',
      recommendations.map((listing) => listing.toJson()).toList(),
    );
  ...
  Future<void> _getCachedFavorites() async {
    ...
    _savedItems = Map<String, bool>.from(
      cachedData ?? {},
    ); // Manejar el caso nulo
    debugPrint(
      'Favoritos recuperados desde Hive: ${_savedItems.length} items.',
    );
  }
  ...
  void _listenListings() async {
    ...
    _listingsSub = _listingService.getListings().listen(
      (listings) async {
      ...
      onError: (error, stackTrace) async {
        debugPrint(
          '[BrowseVM] listings stream failed, falling back to cache: $error',
        );
        final cachedListings = await _listingService.getListings().first;
        _allListings = cachedListings;
        await cacheCatalogSnapshot({
          'listings':
              cachedListings.map((listing) => listing.toJson()).toList(),
        });
        _recomputeVisibleListings();
      },
    );
  ...
  void _listenConfirmedSales() {
    _confirmedSalesSub = _meetupService.watchConfirmedListingIds().listen(
      (ids) {
  ...
  void _updateRegisteredPreferencesFromInteractions() {
    ...
    final interactedListings =
        _listings.where((l) => interactedIds.contains(l.id)).toList();
    ...
    if (allTags.isNotEmpty) {
      ...
      _registeredStylePreferences = topTags
          .take(3)
          .map((e) => e.key)
          .toList(growable: false);
      print(
        '[Prefs] Auto-detected style preferences: $_registeredStylePreferences',
      );
    }
  ...
  Future<void> toggleSave(String itemId) async {
    ...
    _savedItems[itemId] = !(_savedItems[itemId] ?? false);
    final isNowSaved = _savedItems[itemId]!;
    final item = _listings.firstWhere(
      (l) => l.id == itemId,
      orElse:
          () => _allListings.firstWhere(
            (l) => l.id == itemId,
            orElse:
                () => const Listing(
                  id: '',
    ...
    _categoryInteractionCounts[cat] =
        (_categoryInteractionCounts[cat] ?? 0) + 1;
  ...
  List<Listing> get forYouRecommendations {
    ...
    final favoriteIds =
        _savedItems.entries.where((e) => e.value).map((e) => e.key).toSet();
    final allInteractedIds =
        <String>{}
          ..addAll(favoriteIds)
          ..addAll(_viewedItemIds)
          ..addAll(_purchasedItemIds);
    final interactedListings =
        _listings.where((l) => allInteractedIds.contains(l.id)).toList();
    ...
    final similarListings =
        _listings.where((l) {
          if (allInteractedIds.contains(l.id))
            return false; // Do not include duplicates
          for (final tag in l.tags) {
            for (final interactedTag in interactedTags) {
              final similarity = StringSimilarity.compareTwoStrings(
                tag.toLowerCase(),
                interactedTag.toLowerCase(),
              );
              if (similarity >= similarityThreshold) {
                print(
                  '[ForYou] Similar: ${l.title} (tag: $tag) ~ $interactedTag (sim: $similarity)',
                );
                return true;
              }
            }
  ...
  Future<List<Listing>> generateForYouRecommendations() async {
    ...
    Future<List<Listing>> recentSearchesTask = Future(() {
      ...
      return _listings
          .where((listing) => listing.tags.any((tag) => _search.contains(tag)))
          .toList();
    });
    ...
    Future<List<Listing>> tagsTask = Future(() {
      ...
      return _listings
          .where(
            (listing) =>
                listing.tags.any((tag) => interactedTags.contains(tag)),
          )
          .toList();
    });
  ...
  Map<String, int> get forYouNewItemCounts =>
      _recommendationService.getNewItemCounts();

  List<String> get registeredStylePreferences =>
      List.unmodifiable(_registeredStylePreferences);
  ...
  void setRegisteredPreferences({
    ...
    _registeredStylePreferences = stylePreferences
        .map((e) => e.trim())
  ...
  bool _hasStyleMatch(Listing listing) {
    ...
    if (!hasMatch) {
      print(
        '[Match] ${listing.title}: tags=$listingTags vs preferred=$preferred = NO MATCH',
      );
    } else {
      print(
        '[Match] ${listing.title}: tags=$listingTags vs preferred=$preferred = STYLE MATCH ✓',
      );
    }
    return hasMatch;
  }
  ...
  bool _hasSizeMatch(Listing listing) {
    ...
    if (registered.isEmpty) {
      print(
        '[Match] Size check FAILED: Registered size is empty (raw: $_registeredSize)',
      );
      return false;
    }
    ...
    if (!hasMatch) {
      print(
        '[Match] ${listing.title}: item_size=$listingSize vs registered=$registered = NO MATCH',
      );
    } else {
      print(
        '[Match] ${listing.title}: item_size=$listingSize vs registered=$registered = SIZE MATCH ✓',
      );
    }
    return hasMatch;
  }
  ...
  int countRecommendationMatches(Iterable<Listing> recommendations) {
    ...
    print(
      '[Match] ========== Final result: $count / ${recommendations.length} matches ==========\n',
    );
    return count;
  }
    ...
    var filtered =
        _listings
            .where((l) {
              ...
              final matchCond =
                  _condition == 'All' || l.conditionTag == _condition;
  ...
  void _processPendingOperations() async {
    ...
    for (final operation in List.from(_pendingOperations)) {
      ...
      try {
        ...
        debugPrint(
          'Failed to sync operation for item: $itemId. Retrying later.',
        );
      }
    }
  }
  ...
  Future<void> performSearchWithIsolate(String query) async {
    ...
    await Isolate.spawn(_searchIsolateEntry, [
      query,
      _listings,
    ...
    receivePort.listen((filteredResults) {
      debugPrint(
        'Search results received from isolate: ${filteredResults.length} items.',
      );
